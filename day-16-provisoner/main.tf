...
resource "aws_instance" "server" {
  ...
  key_name      = aws_key_pair.example.key_name # calling from line no. 7 key resouce block
  vpc_security_group_ids = [aws_security_group.webSg.id]
  subnet_id              = aws_subnet.sub1.id

# how to connect that created instnace 
  connection {
    type        = "ssh"
    user        = "ubuntu"  # Replace with the appropriate username for your EC2 instance
    # private_key = file("C:/Users/yourname/.ssh/id_rsa")
    private_key = file("~/.ssh/id_rsa")  #private key path
    host        = self.public_ip # same instance that you created that ip it will take
  } 
